...
class OrderTests(APITestCase):
    def setUp(self) -> None:
        """
        Create a new account and create sample category, a product, and a payment type
        """
        url = "/register"
        ...
        url = "/products"
        data = { "name": "Umbrella", "price": 49.19, "quantity": 13, "description": "Shade Yo Self", "category_id": 1, "location": "Pittsburgh" }
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token)
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        url = "/paymenttypes"
        data = {"merchant_name": "American Express", "account_number": "111-1111-1111", "expiration_date": "2024-12-31", "create_date": datetime.date.today()}
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token)
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_add_payment_to_order(self):
        ...
        # Check to make sure there is no cart, which happens when a payment is added.
        url = "/profile/cart"
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token)
        response = self.client.get(url, None, format='json')
    ...
    def test_new_line_item_not_added_to_closed_order(self):
        ...
        # start a new order by adding the second product (product_id: 2) to cart
        url = "/profile/cart"
        data = { "product_id": 2 }
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token)
        response = self.client.post(url, data, format='json')
